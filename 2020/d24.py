...
class Lobby:
    ...
    def execute_daily_flipping(self, times_to_execute: int) -> None:
        for _ in range(times_to_execute):
            neighbours_of_current_tiles = {
                self._move_point_to_direction(point, direction)
            ...
            amount_of_neighbours = {
                point: self._count_black_neighbour_tiles(point)
                for point in potential_tiles
            }
            self.black_tiles = {
                point
                for point in potential_tiles
                if (point in self.black_tiles and 1 <= amount_of_neighbours[point] <= 2)
                or (point not in self.black_tiles and amount_of_neighbours[point] == 2)
            }
    ...
    def _get_first_black_tiles(self, tiles_to_be_flipped: list[str]) -> set[Point]:
        ...
        for steps in tiles_to_be_flipped:
            ...
            while i < len(steps):
                direction, step = (
                    (steps[i : i + 2], 2) if steps[i] in "sn" else (steps[i], 1)
                )
        ...
        return {
            point
            for point, toggle_amount in toggles_per_point.items()
            if toggle_amount % 2 == 1
        }

    def _count_black_neighbour_tiles(self, point: Point) -> int:
        return sum(
            self._move_point_to_direction(point, direction) in self.black_tiles
            for direction in self._DIRECTIONS
        )

    def _move_point_to_direction(self, point: Point, direction: str) -> Point:
        d = self._DIRECTIONS[direction]
...
def main():
    lobby = Lobby(lines)
    print("Part 1:", lobby.amount_of_black_tiles)

    lobby.execute_daily_flipping(100)
    print("Part 2:", lobby.amount_of_black_tiles)
